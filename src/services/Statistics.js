    ...
    const result = await Circulo.aggregate([
      {
        $group: {
          ...
          totalGirls: {
            $sum: {
              $add: ['$girls2', '$girls3', '$girls4', '$girls5', '$girls6'],
            },
          },
          totalMatricula: {
            $sum: {
              $add: ['$matricula2', '$matricula3', '$matricula4', '$matricula5', '$matricula6'],
            },
          },
        },
      },
      {
        $addFields: {
          totalBoys: {
            $subtract: ['$totalMatricula', '$totalGirls'],
          },
        },
      },
      {
...
const getSocialCase = async (req, res) => {
  try {
    const result = await Submision.aggregate([
      {
        $match: { socialCase: true, status: 'matricula' },
      },
      {
        $group: {
